...
Rescattering can manifest as changes to the particle content and changes to the fraction of energy lost to neutrons, \change{which are typically unobservable in the detectors utilized by few-GeV neutrino oscillation experiments.}
...
As the pion mass is reduced towards its physical value, the \asm{energies of the} excited state\asm{s decrease}\sout{energy gap also shrinks}, as the lowest lying excited state is typically a nucleon-pion in a relative $P$-wave.
...
\label{eq:enuhad}
\end{equation}
\noindent where $E_{l}$ is the energy of the outgoing charged lepton, \change{and $E_{\mathrm{kin}}$ and $E_{\mathrm{total}}$ indicate the kinetic and total energies of individual outgoing hadrons.} \asm{\bf [define $E_{\mathrm{kin}}$, $E_{\mathrm{total}}$?]}.
...
\del{Additionally, t}\asm{T}he ratio of the modified to nominal GENIE models is shown in the bottom panel of each plot.}
...
\change{Experimental data from a large number of experiments has disagreed with model predictions in the CC$0\pi$ channel at the 10--30\% level}
\sout{Experimental data from a large number of experiments have disagreed on the 10--30\% level between model predictions and data in the CC$0\pi$ channel}~\cite{garvey_review_2014, Mosel:2016cwa, NuSTEC:2017hzk, Katori:2016yel, ParticleDataGroup:2020ssz}. This has prompted development of {\it ad hoc} systematic uncertainties and empirical model tunings unique to each experiment, with a tendancy to soak up model-data discrepancies into the CC-2p2h channel.
...
Alternative strategies that focus on the inclusive $N\rightarrow X$ contribution~\cite{Hansen:2017mnd,Gambino:2020crt,Fukaya:2020wpp,Bruno:2020kyl}, or the use of two-currents to compute the hadronic tensor~\cite{Liu:1993cv,Liang:2019frk} \change{are likely to} be more fruitful.
...
We have demonstrated that these results produce a significant \asm{shape }change \change{\bf [I'm not sure I would stress shape]} in the predicted neutrino event spectra for T2K (which has similar considerations to Hyper-K) and DUNE experiments. 
