...
For \del{simpler }quantities\del{~with no nontrivial momentum transfer dependence,} such as the spectrum and nucleon axial, scalar and tensor couplings, results at heavier pion masses can help improve the overall precision of the final result provided the extrapolation to the physical pion mass is under control.
...
\del{Decreasing the lattice spacing without a compensating increase in the number of sites
 will result in a decrease in the total volume.}
\del{In practice, it is not possible to change the lattice spacing by an amount that readily.}%
...
%\del{There is no way to continuously vary the lattice spacing without changing the total volume.}
%\del{As the lattice spacing is changed, if the lattice volume is not changed in an exactly compensating way, the physical volume from one lattice spacing to the next will change.}
%\del{As a consequence, t}
\del{\asm{T}he allowed quantized momentum for the nucleon and currents will \asm{therefore} change\del{~with the lattice spacing}, which means the value of the four-momentum transfer $Q$ will not be the same from one \asm{lattice spacing}\del{ensemble} to the next.}
...
Consequently, the continuum extrapolation, which \asm{is} straightforward in principle, is complicated in practice, minimally requiring a combined extrapolation in $Q$ and $a$.
Is it possible to perform a combined extrapolation/interpolation analysis over the full range of \asm{individual }$Q$ while also taking the continuum and physical pion mass limits?


The $z$ expansion \sout{makes use of}\asm{takes advantage of} the analytic structure of QCD\del{,}
 \asm{by }performing a conformal mapping to obtain a new small expansion parameter\del{, and it}\asm{. This technique} has been utilized for decades in meson flavor physics~\cite{Okubo:1971jf}
and is a standard feature in modern LQCD calculations of meson form factors
\sout{for determining}\asm{that constrain} CKM matrix elements.
...
\end{marginnote}%
%-------------------------------------------------------------------------------
This is possible \del{as}\asm{because} there is a rigorous mapping between the quark-level operators in QCD and the higher order Symanzik expansion with hadronic level operators in the EFT.
...
\del{One can of course study effects of truncating the $\chi$PT expansion by working to higher orders in determining the $f_k$ coefficients and studying the impact on the analysis.
 {\bf [don't understand this sentence]}}
...
\end{align}
 which starts at $O(x)$.
\del{For large $Q^2$, the relation is instead expanded in terms of $x^{-1}$.}
...
and then substituting equation~(\ref{eq:Q2toz}) to convert powers of
 \asm{$x$}\del{$Q^2+t_0$} into powers of $z$.
All dependence on the dimension is absorbed into powers of
 $t_c-t_0 \asm{\sim}\del{\propto} m_\pi^2$.
...
If we consider the $Q^4$ expansion of Equation~\eqref{eq:F_Q_power} with
 a truncation at the leading chiral and discretization corrections\asm{, we get}
\begin{align}
...
\pagebreak
\del{
Even though we cannot rule out the possiblity of an undetermined systematic correction,
 where we have most importantly focused on the excited state contamination,
 the consistency of the LQCD predictions imply that these results are unlikely to change,
 and will continue to be refined and substantiated in the upcoming years.
}
While it is possible that the lattice community will uncover additional systematic uncertainties in the calculation of the axial form factor, given the current state of understanding in the field, we believe this is unlikely.  The consistency of the various incomplete LQCD results at the physical pion mass lend confidence to this assessment, and \del{it is most likely }these results will continue to be refined and substantiated.
%while it is possible that the lattice community.
%\asm{\bf [orphaned, move up to first paragraph? or combine with previous?]}
\del{Given the state of the field and the rapid advances recently made, we anticipate this excited state issue will be definitively resolved for elastic nucleon form factor calculations within a year or two,}
\change{
Given the state of the field and the rapid advances recently made, we anticipate that \sout{these issues will be definitively resolved }\asm{the systematic uncertainties will be fully understood}\sout{for elastic form factor calculations }within a year or two,}
 thus enabling precise determinations of the quasielastic axial form factor and the elastic electric and magnetic form factors\asm{.
At this point, LQCD will be able to resolve}\del{ that will resolve} the \asm{experimental and phenomenological} discrepancies reviewed in Section~\ref{sec:sof}.
