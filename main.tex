...
\done{\asm{Rescattering can manifest as changes to the particle content} and \asm{as }change\asm{s to} the fraction of energy lost to neutrons, which are typically unobserved in the detectors used.}
\change{The presence of multiple interactions channels and the addition nuclear effects significantly complicates the analysis of data from neutrino experiments, and gives rise to a major source of systematic uncertainty.}
...
\end{marginnote}%
$\nu A$ scattering is the lack of data \change{with which} to benchmark parts of the calculation. % against.
For example, neutrino quasielastic scattering ($\nu_{l} + n \rightarrow l^{-} + p$ or $\bar{\nu}_{l} + p \rightarrow l^{+} + n$) is the simplest of the relevant hard scattering processes, and dominates the neutrino cross section below energies of $\approx$1 GeV. However, modern experiments using nuclear targets are unable to measure it without significant nuclear effects~\cite{garvey_review_2014, NuSTEC:2017hzk}. \change{Instead, they measure a specific final state, such as one muon and no pions, which will be dominated by quasielastic processes, but which will have significant contributions from resonant pion production events where the pion has rescattered in the nucleus and has either been absorbed or has lost sufficient energy to be below detection threshold.}
\change{Given the challenge to benchmark neutrino cross-section models for quasielastic scattering (and other hard-scattering processes) with new $\nu A$ datasets, experiments and theorists have relied heavily on sparse data from the 1960--1980's from several bubble chamber experiments which used H$_{2}$ or D$_2$ targets~\cite{zeller12, ParticleDataGroup:2020ssz}.}
...
These data will \change{not be considered} %be ignored
in this review.
...
LQCD provides a theoretical \change{method} %alternative
...
{\color{ao(english)} \change{The need for this is clear:} sparse data from deuterium bubble-chamber experiments do not constrain
...
\change{Given that a modern $\nu N$ scattering experiment is extremely unlikely, LQCD is the only viable method to improve our understanding of the axial form factor to the required level of precision.}
%Ideally, the lack of precision in the axial form factor would be rectified by a modern $\nu N$ scattering experiment.}
}

\change{Already, } one extremely striking feature of LQCD calculations is the strong preference for a slower
fall off of the form factor with increasing $Q^2$ than predicted by phenomenological determinations from experiment (\change{Section~\ref{sec:lqcd_results}}).
...
The size of the \change{upper} red band with respect to the \change{lower, large} green band demonstrates the uncertainty reduction from replacing the deuterium scattering axial form factor with one obtained from LQCD. \change{\sout{and}} The significant change in normalization is due to the slower fall off of the axial form factor.
Additionally, the size of the red band demonstrates the relevance of the tension in vector form factors, characterized by the difference between the black and blue curves, \change{with the uncertainty from LQCD a sub-dominant correction}.
...
\caption{
 ...
 for various choices of parameterization explained in the bullet list provided in the \change{Section~\ref{sec:sof}}.
