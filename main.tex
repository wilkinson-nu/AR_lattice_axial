...
Even if only physical pion mass ensembles were used with sufficiently large volumes that the volume corrections are negligible, there would still be an issue related to the continuum extrapolation.
As the lattice spacing is changed, if the lattice volume is not changed in an exactly compensating way, the physical volume from one lattice spacing to the next will change.  As a consequence, the available momentum for the nucleon and currents will change, which means the value of the four-momentum transfer $Q$ will not be the same from one lattice spacing to the next.  Consequently, the continuum extrapolation, which straightforward in principle, is complicated in practice, requiring a minimum of a combined extrapolation in $Q$ and $a$.
