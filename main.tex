...
For \sout{simpler }quantities\asm{~with no nontrivial momentum transfer dependence}, such as the spectrum and nucleon axial, scalar and tensor charges, results at heavier pion masses can help improve the overall precision of the final result provided the extrapolation to the physical pion mass is under control.
For the form factors,
 \sout{even if only physical pion mass ensembles were used with sufficiently large volumes that the volume corrections are negligible,
 there would still be an issue related to the continuum extrapolation.}
 \asm{there is still an issue related to the continuum extrapolation even when using only ensembles with physical pion mass and large volumes with negligible volume corrections.}
\asm{%
The reason is that the spatial volume is given by the lattice spacing times
 the number of spatial sites.
Decreasing the lattice spacing without a compensating increase in the number of sites
 will result in a decrease in the total volume.}
\sout{As the lattice spacing is changed, if the lattice volume is not changed in an exactly compensating way, the physical volume from one lattice spacing to the next will change.}
\sout{As a consequence, t}\asm{T}he allowed quantized momentum for the nucleon and currents will \asm{therefore} change\asm{~with the lattice spacing}, which means the value of the four-momentum transfer $Q$ will not be the same from one \sout{lattice spacing}\asm{ensemble} to the next.
Consequently, the continuum extrapolation, which \asm{is} straightforward in principle, is complicated in practice, requiring a\asm{t} minimum \sout{of }a combined extrapolation in $Q$ and $a$.
...
The $z$ expansion~\cite{Bhattacharya:2011ah} is often used to improve the extrapolation/interpolation in $Q^2$.
The $z$ expansion \sout{makes use of}\asm{takes advantage of} the analytic structure of QCD\del{,}
 \asm{by }performing a conformal mapping to a obtain a new small expansion parameter\del{, and it}\asm{. This technique} has been utilized for decades in meson flavor physics~\cite{Okubo:1971jf}
...
\asm{One can of course study effects of truncating the $\chi$PT expansion by working to higher orders in determining the $f_k$ coefficients and studying the impact on the analysis.
...
\end{align}
 which starts at $O(x)$.
\sout{For large $Q^2$, the relation is instead expanded in terms of $x^{-1}$.}
...
and then substituting equation~(\ref{eq:Q2toz}) to convert powers of
 \asm{$x$}\sout{$Q^2+t_0$} into powers of $z$.
