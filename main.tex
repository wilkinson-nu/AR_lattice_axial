...
Rescattering can manifest as changes to the particle content and changes to the fraction of energy lost to neutrons, which are typically unobserved in the commonly used detectors.
...
Two different parameterizations of the form factor, normalized by a dipole parameterization,%
\begin{marginnote}
...
The success in calculating the threshold value $g_A$ motivates current efforts
to map out $F_A(Q^2)$ of importance to the long-baseline neutrino program.
...
A striking feature of LQCD calculations of $F_A(Q^2)$
...
The LQCD uncertainty is also noticeably smaller than the green band that arises from the deuterium scattering determination of $F_A(Q^2)$.
...
However, twisted mass fermions break isospin symmetry at finite lattice spacing, which causes some complications now that LQCD results are precise enough to resolve isospin breaking corrections from $m_d-m_u$ and%
\begin{marginnote}
...
\sout{In order to determine the mass of the nucleon (or any hadron), we rely on the construction of}
\asm{Hadron masses are determined from LQCD by constructing} two-point correlation functions \sout{that are constructed} in a mixed time-momentum representation\del{,}\asm{.}
\del{ most commonly utilizing}\asm{A common strategy uses} spatially local creation operators (sources) and momentum space annihilation operators (sinks)\asm{, taking advantage of momentum conservation to select the source momentum}.
...
\end{marginnote}%
%-------------------------------------------------------------------------------
\sout{Momentum conservation selects only the state $n(\mathbf{p})$ in the sum over all states enabling the use of this mixed spatial creation operator and momentum annihilation operator.}
As we will discuss in more detail below, it is more desirable to instead \del{creation}\asm{build both source and sink} operators
in momentum space\sout{ rather than position space}.
\del{They}\asm{Momentum space sources} are not commonly used as they are significantly more \asm{numerically} expensive \del{numerically }to generate.
...
\sout{A well known challenge \asm{that}\del{which} complicates the analysis is that f}%
\asm{F}or nucleon two-point functions, the S/N ratio degrades exponentially at large Euclidean time~\cite{Lepage:1989hd}
\begin{equation}
\lim_{{\rm large}\ t} {\rm S/N}%\frac{\mathrm{Signal}}{\mathrm{Noise}}
    \propto \sqrt{N_{\mathrm{sample}}} e^{-(m_{\mathrm{N}} - \frac{3}{2}m_\pi)t}\, \del{,}\asm{.}
\end{equation}
\sout{which adds an extra source of excited state systematic uncertainty that must be quantified.}
\change{In} the region in time when the ground state begins to saturate the correlation functions, \asm{typically around}\del{at} $t\approx 1$~fm, the noise \change{becomes} significant, \asm{which makes}\del{making} the correlation functions in this region susceptible to correlated fluctuations \asm{that}\del{which} can bias a simplistic single-state analysis.
\asm{This forces measurements to be made at Euclidean times where
 excited state contamination is still appreciable,
 which adds an extra source of systematic uncertainty.}
\del{Further, a}\asm{A}s the pion mass is reduced towards its physical value, the \asm{energies of the} excited state\asm{s decrease}\sout{energy gap also shrinks}, as the lowest lying excited state is typically a nucleon-pion in a relative $P$-wave.
At the same time, the energy scale \asm{that}\del{which} governs the exponential degradation of the signal also grows.
The former issue means calculations must be performed at larger Euclidean time to suppress the \sout{smaller gapped}\asm{slowly decaying} excited states\asm{,} and the latter issue means we need exponentially more statistics to obtain a fixed relative uncertainty at a given \asm{Euclidean} time.
\asm{\bf [who is this the rest of this paragraph written for?
 seems unnecessary, since LQCD people will already know it
 and neutrino people won't need to know it.
 remove?]}
In order to boost statistics, \sout{given the numerical cost of generating more configurations, }time and spatial translation invariance are used to generate sources at several choices of $t_0$ and $\mathbf{x}_0$\del{, which}\asm{~that} are then translated back to the origin and averaged together.  For the nucleon, it was observed that hundreds of sources on each configuration, using an anisotropic clover-Wilson ensemble~\cite{HadronSpectrum:2008xlg}, still showed approximate $\sqrt{N_{\rm sample}}$ improvement of the stochastic uncertainty as a function of the number of sources per configuration~\cite{Beane:2009kya}.

The most common method of constructing three-point correlation functions follows a \del{similar }strategy\asm{~similar to the two-point correlation functions}, beginning with spatially local sources.
...
\end{align}
where $g_{n,m}^\G$ are the matrix elements of interest, and in principle, all other quantities can be determined from \del{the }two-point function\asm{s}, a point we will return to.
\asm{\bf [this $p_n$, $p_m$ notation might be confusing because the spectral decompositions of $|n\rangle$ and $|m\rangle$ are not explicitly written]}
Often, the sink is projected to zero momentum, $\mathbf{p}_n=0$, \del{for which}\asm{and} momentum conservation \sout{gives the momentum of the}\asm{selects an} incoming state \del{to be}\asm{with} $\mathbf{p}_m = -\mathbf{q}$.%
%-------------------------------------------------------------------------------
\begin{marginnote}
...
whose source is \sout{constructed}\asm{obtained} by taking \del{the }forward propagators from the origin and contracting \asm{the spin and color indices for all but one quark operator}\sout{all but one spin and color index at the sink time, $\tsep$}.
\asm{In the case of the nucleon three-point correlation function,
...
\asm{\bf [like above, who is this paragraph written for?]}
...
%\asm{Sequential sources are relatively expensive, since a new sequential source
% is needed every time one the sequential propagator contraction changes.}
...
The most significant challenge \asm{for}\del{in} determining the nucleon matrix elements and subsequent form factors is dealing with the excited state contamination, an issue that is compounded by the degrading S/N.
If the nucleon two-point function is becoming saturated by the ground state at $t\approx1$~fm, \del{the }ideal\asm{ly} three-point function\asm{s} would use values of $\tsep\asm{\gtrsim}\del{\approx}2$~fm.
\asm{However, the S/N ratio of the three-point functions decays more rapidly than the two-point functions.}
In practice, a few values of $\tsep$ in the range $0.8 \lesssim \tsep \lesssim 1.5$~fm are used \sout{because the S/N ratio of the three-point functions decays more rapidly than the two-point function, }and a\del{n extrapolation to large $\tsep$}\asm{~fit to the time dependence} is used to isolate the ground state matrix elements.
\sout{However, using }\asm{Fits that allow for }just one excited state \sout{in the analysis }require\del{s} three values of $\tsep$ to \sout{have a one degree-of-freedom fit to extract the ground state matrix element}\asm{not be overconstrained}.
Most results have been generated with three or fewer values of $\tsep$.
Ref.~\cite{Chang:2018uxx} utilized many values of $\tsep$ to determine $g_{\mathrm{A}}$, \change{and} a few \change{other} groups have begun advocating for the use of many values of $\tsep$, including small values, to improve \sout{our ability to understand and }control \asm{over} the excited state contamination~\cite{Hasan:2019noy,Alexandrou:2019brg,He:2021yvm}.
Ref.~\cite{He:2021yvm} utilized 13 values of $\tsep$, which allow\del{s}\asm{ed} for a systematic study of the uncertainty associated with \asm{the truncation of $\tsep$ and a fit to a 5-state model}\sout{truncating early and/or late values of $\tsep$ in the analysis, as well as providing sufficient data to perform up to a complete 5-state fit}.

At non-zero momentum, the \asm{tradeoff between }excited states \asm{and S/N }become\asm{s} more problematic.
While the energy associated with the signal grows with the momentum, the energy scale associated with the noise is independent of the momentum\del{ and thus t}%
\asm{. T}heir difference, which is the energy scale that governs the decay of the S/N, \asm{therefore} grows with increasing momentum.
...
\sout{For boosted nucleons, p}\asm{P}arity is also no longer a good quantum number \asm{for boosted nucleons} and so the \asm{matrix elements couple to both}\sout{correlation function begins to mix} even and odd parity states\sout{ through the matrix elements}.  Such contamination can be handled through a variational method that incoporates even and odd parity nucleons~\cite{Stokes:2013fgw,Stokes:2018emx,Stokes:2019zdd}.



Recent results have uncovered some additional aspects of excited state contamination:
\begin{itemize}[leftmargin=*]
\item \del{T}\asm{Nucleon t}wo-point functions constructed as in Equation~\eqref{eq:2pt} are insufficient to reliably determine any of the excited states.  Different choices of $\tmin$ and different reasonable priors in a Bayesian analysis support excited states that differ by several sigma, also resulting in sensitivity of the ground state spectrum~\cite{Park:2021ypf,He:2021yvm} and matrix elements~\cite{Jang:2019vkm,Gupta:2021ahb}.
...
\item For zero-momentum transfer,
 ...
 the use of \asm{analysis techniques such as }the summation method~\cite{Maiani:1987by}
 or a variant of the Feynman-Hellman
 method~\cite{deDivitiis:2012vs,Bouchard:2016heu}\del{,}
 \del{which is closely related to the summation method,}%
 \sout{it is understood that the excited states become significantly suppressed compared to the three-point function~}%
 \asm{can significantly suppress the excited states}~\cite{Capitani:2012gj,He:2021yvm}.
...
\sout{$F_{\rm A}(Q^2)$ can be isolated in LQCD calculations with particular kinematic choices that are not contaminated by the induced pseudoscalar form factor, see for example Ref.~\cite{Gupta:2017dwj}.}
Given the challenges in identifying all the sources of systematic uncertainty in the calculation of $g_{\mathrm{A}}$, particularly the excited state contamination, it is prudent to perform cross checks of observables \del{to}\asm{that} test for consistency of the results.
The validity of the PCAC relation,
\begin{align}
 \partial^\mu A^{a}_{\mu}(x) = 2 m_q P^{a}(x),
 \label{eq:pcac}
\end{align}
provides a complex consistency check\asm{~at nonzero $Q^2$}.
...
The axial \asm{form factor} contributions are largely dominated by loop-level $N\pi$ excited states
 with a highly suppressed tree-level correction.
The correction to the axial \asm{form factor}\sout{current} is nearly independent of $Q^2$.
On the other hand, corrections to the induced pseudoscalar \asm{form factor} are
...
The contamination to the pseudoscalar current is redundant with the
 \asm{chiral corrections to the} axial and induced pseudoscalar
 \del{chiral corrections}\asm{form factors} and can be obtained
 by application of the PCAC relation.

Scrutiny of the LQCD data has demonstrated many of the features
 \sout{that were expected from}\asm{predicted by} chiral perturbation theory.
...
The \del{primary}\asm{main} excited state\asm{s that contaminate}\del{ contaminations to} the temporal axial form factor matrix elements
 were shown to be driven by two specific $N\pi$ states,
 characterized by a transition\del{ through an axial current}
 of the nucleon state to an $N\pi$ excited state \asm{through the axial current} or vice versa~\cite{Jang:2019vkm}.
...
This has led to new analysis strategies that more carefully deal with the $N\pi$ excited state with pion-pole contributions\asm{
~and pull information about the excited states from the temporal axial current.}\del{,}
\asm{These new strategies} yield\del{ing} minimal violations of PCAC\del{ that include such matrix elements, as well as}\asm{~and an} improved understanding of excited state contamination on matrix elements sensitive to $F_{\rm A}(Q^2)$ or $\tilde{F}_{\rm P}(Q^2)$\del{,}\asm{.}
\del{ which previously}\asm{Previously, these matrix elements} exhibited deviations as large as $40\%$ from the PPD or GGT
 \del{ansatz}\asm{relations} at low $Q^2$\del{,} where \asm{they were}\del{it was} expected to work best~\cite{Bali:2014nma,Gupta:2017dwj}.
...
\sout{This is especially true considering the agreement between LQCD and experiment
...
\asm{Despite the agreement between LQCD and experiment for the axial radius,
 LQCD prefers an axial form factor that fall offs with slower $Q^2$ than experiment,
...
LQCD results for $F_A(Q^2)$ and the Dirac and Pauli form factors are often determined up to $Q^2\approx1-2$~GeV$^2$.
This extends beyond the range of validity of $\chi$PT~\cite{Gasser:1984gg,Jenkins:1990jv,Bernard:1995dp}, which may converge up to $Q\approx300$~MeV.%
% FOOTNOTE ---------------------------------------------------------------
\footnote{The range of validity of $\chi$PT seems to be at best $m_\pi\lesssim300$~MeV~\cite{Beane:2004ks,Walker-Loud:2008rui} with some indications the convergence is troubled at the physical pion mass~\cite{Walker-Loud:2019cif,Drischler:2019xuo}.}
The LQCD results are also obtained at finite lattice spacing and finite volume, leading to additional systematic corrections to the observables.
For small momentum, small pion mass, small lattice spacing~\cite{Symanzik:1983dc,Symanzik:1983gh,Sharpe:1998xm} and large volumes~\cite{Gasser:1986vb}, these systematics can all be captured through the EFT framework~\cite{Weinberg:1978kz}.
...
Is it possible to perform a combined extrapolation/interpolation analysis over the full range of $Q^2$ while also taking the continuum and physical pion mass limits?




{\color{ao(english)}OLD BELOW}
...
\label{eq:enuqe}
\end{equation}
\noindent where $m_l$ is the mass of the outgoing lepton, $m_{i}$ is the mass of the initial state nucleon, and $m_{f}$ is the mass of the final state nucleon. As this variable assumes quasielastic kinematics, it is applied to a signal sample of CC0$\pi$ events.\footnote{Note that in recent analyses, T2K has included samples \del{including}\asm{with} a single charged pion using a modified version of Equation~\eqref{eq:enuqe}~\cite{T2K:2017rgv, T2K:2019bcf, T2K:2021xwb}.}%
\begin{marginnote}
...
\asm{, for} which the axial form factor is relevant\asm{,}\del{ for}
 \sout{clearly }make up the majority of T2K's CC0$\pi$ sample\del{,}
 in both the oscillated and unoscillated fluxes.
\sout{It is also clear that t}\asm{T}he tuned LQCD values \del{has}\asm{have a} significant effect on the total expected event rate in both cases, on the order of 20\%.

Also clear from Figure~\ref{fig:t2k_impact} is that the change in \asm{both }the total\del{, or}\asm{~and} CCQE-contributed event rates is not purely a normalization change, \asm{but instead depends on neutrino energy }and is different for the near and far detector\sout{ spectra}.
...
Experiments with higher neutrino beam energies typically do not limit their analyses to CC0$\pi$ events or use Equation~\ref{eq:enuqe} for reconstructing the neutrino energy. Instead, they use all charged-current events (CC-inclusive), and reconstruct the neutrino energy through a combination of particle identification and tracking\sout{, and}\asm{~together with} calorimetry.
...
\label{eq:enuhad}
\end{equation}
\noindent where $E_{l}$ is the energy of the outgoing charged lepton\asm{\bf [define $E_{\mathrm{kin}}$, $E_{\mathrm{total}}$?]}.
...
\begin{figure}[htbp]
  ...
  \vspace{11pt}
  \caption{The $\nu_{\mu}$--$^{40}$Ar CC-inclusive event rates per ton (kiloton) per $1\times10^{21}$POT at DUNE's near (far) detector site, shown as a function of $E^{\mathrm{rec,\;had}}_{\nu}$. The GENIE~\cite{Andreopoulos:2009rq, GENIE:2021npt} nominal event rate (blue solid line) is produced using the GENIEv3 10a\_02\_11a tune to nucleon data~\cite{GENIE:2021zuu} and the DUNE flux~\cite{Abi:2020evt}\del{, and t}%
...
\del{Additionally, t}\asm{T}he ratio of the modified to nominal GENIE models is shown\asm{~in the bottom panel of each plot}.}
...
The modification to the axial form factor based on the LQCD results from the CalLat collaboration described in Section~\ref{sec:callatdata} \sout{has an approximately 10\% effect to}\asm{results in an approximately 10\% enhancement to} the total predicted event rate at both the near and far detectors\sout{, with different shape dependence, as in the T2K case.}%
\asm{. As in the case with the T2K event rate, the enhancement has a nontrivial neutrino energy dependence.}
...
Experimental data from a large number of experiments \sout{has found disagreements}\asm{have disagreed} on the 10--30\% level between model predictions and data in the CC$0\pi$ channel~\cite{garvey_review_2014, Mosel:2016cwa, NuSTEC:2017hzk, Katori:2016yel, ParticleDataGroup:2020ssz}\sout{, prompting}%
\asm{. This has prompted} development of {\it ad hoc} systematic uncertainties and empirical model tunings \sout{by individual }\asm{unique to each }experiment\del{s}, with a tendancy \sout{has been }to soak up model-data discrepancies into the CC-2p2h channel.
...
If they are, then the LQCD results \sout{indicate}\asm{imply} that the nucleon axial form factor is significantly different than what has been extracted phenomenologically, as indicated in Figure~\ref{fig:gaq2_overlay}.
...
First, these methods enable the use of multi-hadron creation and annihilation operators
 ...
 \del{which can}\asm{that} prominantly decay\asm{s} to an $N\pi\pi$ state,
 or otherwise.
...
This enables one to construct useful linear combinations of correlation functions that eliminate\asm{,} for example, the induced pseudoscalar contribution to an axial three point function, \asm{which} simplif\asm{ies}\del{ying} the analysis~\cite{Meyer:2017ddy}.
...
\sout{It is likely that a}\asm{A}lternative strategies \del{which}\asm{that} focus on the inclusive $N\rightarrow X$ contribution~\cite{Hansen:2017mnd,Gambino:2020crt,Fukaya:2020wpp,Bruno:2020kyl}, or the use of two-currents to compute the hadronic tensor~\cite{Liu:1993cv,Liang:2019frk} will \asm{likely }be more fruitful.
...
These results introduce the exciting possibility of \asm{using }LQCD calculations to tackle other quantities that are not easily \asm{accessible }experimentally\sout{ accessible}, or for which tensions between measurements, or competing models, exist.
In this review\asm{,} we discussed \sout{using }LQCD \sout{to calculate}\asm{calculations of} nucleon form factors as a function of momentum transfer, which are of particular interest to the few-GeV neutrino experimental program.
\del{Notable tensions exist in current parameterizations of the vector form factors, but h}\asm{H}ere we focused on the axial form factor, $F_{\mathrm{A}}(Q^2)$, which is of primary importance because current parameterizations are simplistic and rely on a handful of low-statistics $\nu N$ bubble chamber measurements.
\asm{However, notable tensions exist in current parameterizations of the vector form factors too.}
$F_{\mathrm{A}}(Q^2)$ cannot be cleanly measured with existing experiments \del{which}\asm{that} use heavier nuclear targets both for safety reasons and to increase the event rate, so LQCD offers a novel path to this important quantity.
We have compared $F_{\mathrm{A}}(Q^2)$ calculations from a variety of different LQCD collaborations using different approaches and techniques, and shown them to be in good agreement with each other, but crucially, in poor agreement with the simple dipole model tuned to historic $\nu N$ data \asm{that are }currently relied upon.
...
We have demonstrated that these results produce a significant \asm{shape }change in the predicted neutrino event spectra for T2K (which has similar considerations to Hyper-K) and DUNE experiments.
...
We additionally discussed a number of ways \del{in which}\asm{that} current calculations can be improved and validated, and to increase confidence that the LQCD results are not subject to an uncontrolled systematic uncertainty.
Finally, we discussed a number of other quantities that are important to neutrino oscillation experiments in the few-GeV energy regime \asm{and} that LQCD can provide first principles predictions for.
