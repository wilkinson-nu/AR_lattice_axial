...
\item Twisted mass fermions are a variant of Wilson fermions that exploits the approximate $SU(2)$ chiral symmetry of QCD to introduce a twisted quark mass term, $i\mu\g_5 \tau_3$ \change{which can be used to automatically remove the leading $\mathrm{O}(a)$ discretization effects~\cite{Frezzotti:2003ni} at the expense of introducing isospin breaking at finite lattice spacing.}
\sout{At \asm{maximal twist}, the bare quark mass is cancelled by the $1/a$ additive quark mass, leaving $\mu$ as the only contribution through $\mathrm{O}(a)$ to the physical quark mass.  All observables are automatically \asm{$\mathrm{O}(a)$ improved} at maximal twist.
...
\asm{\bf [who is this the rest of this paragraph written for?
 ...
 remove?]}
In order to boost statistics, time and spatial translation invariance are used to generate sources at several choices of $t_0$ and $\mathbf{x}_0$ that are then translated back to the origin and averaged together.  For the nucleon, it was observed that hundreds of sources on each configuration, using an anisotropic clover-Wilson ensemble~\cite{HadronSpectrum:2008xlg}, still showed approximate $\sqrt{N_{\rm sample}}$ improvement of the stochastic uncertainty as a function of the number of sources per configuration~\cite{Beane:2009kya}.
...
\change{When applied to the axial current matrix element of the nucleon, it yields the Goldberger-Treiman relation, which is usually expressed in the $Q^2\approx m_\pi^2$ region.  For arbitrary $Q^2$, it is sometimes referred to as the GGT}
\sout{The generalization of this relation to the nucleon form factors
 yields the GGT}%
 \begin{marginnote}
...
This has led to new analysis strategies that more carefully deal with the $N\pi$ excited state with pion-pole contributions\asm{
  ~and pull information about the excited states from the temporal axial current.} \change{\bf[``pull'' seems informal?]}
...
Despite the agreement between LQCD and experiment for the axial radius,
 LQCD \change{predicts} \del{prefers} an axial form factor that \change{falls off more slowly as a function of $Q^2$} than experiment,
