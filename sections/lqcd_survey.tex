...
Though the axial charge and radius are useful for connecting with low-energy applications,
 ...
 \done{Something has gone wrong here. What the main deliverables are can't be the especially true thing you intended.}
 \asm{reordered first two sentences}
...
\new{The form factor data from the remaining ETMC ensemble,
...
\done{I would suggest ``modified'' rather than spoiled, which seems too emotive}
\new{These claims could be modified by systematic effects
...
The LHPC~\cite{Hasan:2017wwt} and PACS~\cite{Ishikawa:2021eut} collaborations
 ...
 \done{Do we need citations for these two collaborations? It seems difficult for a reader to find out more...}
 \asm{The two collaborations are working together (I am a part too),
 there aren't many references to cite yet though.
 Moved the citations up from the end of the sentence and added a few more.}
...
The fits to one physical mass ensemble from the CalLat collaboration are used
 to make statements about the phenomenological impact of the LQCD results in Sec.~\ref{sec:impact}.
...
\right).
\end{align}
\asm{what's the best way to write this out without removing too much precision?
check this for pos-defness}
